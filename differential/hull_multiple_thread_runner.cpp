// NeoAlzette differential HULL hybrid multi-process/multi-thread campaign runner.
//
...
namespace neoalzette_runner
{
    ...
    constexpr const char* kAnalysis = "xor_differential";
    constexpr const char* kTimeOption = "--hull-time-limit";
    constexpr const char* kInputAOption = "--fix-input-da";
    constexpr const char* kInputBOption = "--fix-input-db";
    constexpr const char* kInputAField = "dA_in";
    constexpr const char* kInputBField = "dB_in";
    constexpr const char* kOutputAField = "dA_out";
    constexpr const char* kOutputBField = "dB_out";
    constexpr const char* kCharacteristicsOption = "";
    ...
    static int run_hull_api_job( const ScheduledJob& job, std::optional<int> cpu_id )
    {
        try
        {
            ...
            neoalzette_diff_milp::run_forest_hull_search_from_argv( static_cast<int>( argv_values.size() ), argv_ptrs.data() );
            return 0;
        }
    ...
    static std::vector<AttemptRecord> extract_attempts( const ScheduledJob& job )
    {
        ...
        for ( const Json& attempt : *attempts_value->array() )
        {
            ...
            if ( auto value = numeric_or_string_member( attempt, "probability_sum" ) )
            {
                record.hull_primary_measure = *value;
                record.hull_absolute_measure = std::fabs( *value );
            }
            if ( auto value = number_member( attempt, "effective_weight" ) ) record.hull_effective_weight = *value;
