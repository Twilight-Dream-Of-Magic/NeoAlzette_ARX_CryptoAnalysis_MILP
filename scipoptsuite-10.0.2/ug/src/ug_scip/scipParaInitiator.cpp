      ...
      )
{
   ...
   else
   {
      ...
      {
         std::cout << *this << "Warning: SCIP was interrupted because the memory limit was reached" << std::endl;
         return false;
      }
      if( paraParams->getRealParamValue(UG::TimeLimit) > 0.0 &&
      ...
      )
{
                                 ...
                                 /** the second argument is problem file name */
   {
      ...
      else if( strcmp(argv[i], "-t" ) == 0 )
      {
         ...
         if( i < argc )
         {
            double timeLimit = atof(argv[i]);
            paraParams->setRealParamValue(UG::TimeLimit, timeLimit);
          }
          else
          {
             std::cerr << "missing solution filename after parameter '-t'" << std::endl;
             exit(1);
          }
      }
      else if( strcmp(argv[i], "-m" ) == 0 )
      {
         ...
         if( i < argc )
         {
            double memoryLimit = atof(argv[i]);
            paraParams->setRealParamValue(MemoryLimit, memoryLimit);
          }
          else
          {
             std::cerr << "missing solution filename after parameter '-m'" << std::endl;
             exit(1);
          }
      }
#if ( defined(_COMM_PTH) || defined(_COMM_CPP11) )
   ...
   (void)ParaInitiator::init(inParaParams, argc, argv);
   ...
   includeUserPlugins(scip, logname == nullptr ? logname : lognameinstance.c_str(), quiet, comment);
   ...
   if( SCIPgetParam(scip, "constraints/orbisack/forceconscopy") != NULL )
   {
      SCIP_CALL( SCIPsetBoolParam(scip, "constraints/orbisack/forceconscopy", TRUE) );
   }
   if( SCIPgetParam(scip, "constraints/symresack/forceconscopy") != NULL )
   {
      SCIP_CALL( SCIPsetBoolParam(scip, "constraints/symresack/forceconscopy", TRUE) );
   }

   /* depending on the SCIP version, only the parameters for orbitope, or orbitope_full and orbitope_pp, exist */
   if( SCIPgetParam(scip, "constraints/orbitope/forceconscopy") != NULL )
   {
      SCIP_CALL( SCIPsetBoolParam(scip, "constraints/orbitope/forceconscopy", TRUE) );
   }
   if( SCIPgetParam(scip, "constraints/orbitope_full/forceconscopy") != NULL )
   {
      SCIP_CALL( SCIPsetBoolParam(scip, "constraints/orbitope_full/forceconscopy", TRUE) );
   }
   if( SCIPgetParam(scip, "constraints/orbitope_pp/forceconscopy") != NULL )
   {
      SCIP_CALL( SCIPsetBoolParam(scip, "constraints/orbitope_pp/forceconscopy", TRUE) );
   }
   ...
   else if( paraParams->getBoolParamValue(Quiet) )
   {
   ...
   {
      if( logname != nullptr || quiet )
      {
         if( logname != nullptr )
         {
            logfile = fopen(lognameinstance.c_str(), "a"); // append to log file */
            if( logfile == NULL )
            {
               THROW_LOGICAL_ERROR3("cannot open log file <", lognameinstance.c_str(), "> for writing");
            }
         }
   ...
   if( probname != NULL )
   {
      ...
      if( !(settingsName == NULL && settingsNameRoot == NULL && settingsNameLC == NULL ) )
      {
         ...
         /** user include plugins */
         includeUserPlugins(paramScip, logname == nullptr ? logname : lognameinstance.c_str(), quiet, comment);
      }
      ...
      if( settingsName != NULL )
      {
         ...
         if( getAbsgapValue(paramScip) > MINEPSILON || getGapValue(paramScip) > MINEPSILON )
         {
            if( paraParams->getIntParamValue(RampUpPhaseProcess) == 2 )
            {
               ...
               std::cout << *this << "** RampUpPhaseProcess is switched to 1, since gap for solvers is specified" << std::endl;
            }
         }
      }
      else
      ...
      if( settingsNameRoot != NULL )
      {
         ...
         if( getAbsgapValue(paramScip) > MINEPSILON || getGapValue(paramScip) > MINEPSILON )
         {
            ...
            {
               paraParams->setIntParamValue(RampUpPhaseProcess, 1);
               std::cout << *this << "** RampUpPhaseProcess is switched to 1, since gap for root node is specified" << std::endl;
            }
         }
      }
      else
      ...
      if( settingsNameLC != NULL )
      {
         // SCIP_CALL( SCIPresetParams(scip) );
         SCIP_CALL_ABORT( SCIPreadParams(scip, settingsNameLC) );
         std::cout << *this << "** applied absolute gap = " << getAbsgapValue() << std::endl;
         std::cout << *this << "** applied relative gap = " << getGapValue() << std::endl;
         if( getAbsgapValue() > MINEPSILON || getGapValue() > MINEPSILON )
         {
            if( paraParams->getIntParamValue(RampUpPhaseProcess) == 2 )
            {
               paraParams->setIntParamValue(RampUpPhaseProcess, 1);
               std::cout << *this << "** RampUpPhaseProcess is switched to 1, since gap is specified" << std::endl;
            }
         }
      }
      ...
      {
         std::cout << *this << "error reading file <" << probname << ">" << std::endl;
      ...
      if( paraParams->getBoolParamValue(NoPreprocessingInLC) )
      {
         ...
         {
            SCIP_CALL_ABORT( SCIPsetIntParam(scip, "presolving/maxrounds", 0) );
            std::cout << *this << "No LC presolving is specified." << std::endl;
         }
         else
         {
            std::cout << *this << "Default LC presolving (default)." << std::endl;
         }
      }
      else
      {
         ...
         {
            SCIP_CALL_ABORT( SCIPreadParams(scip, settingsNameLC) );
            std::cout << *this << "LC presolving settings file is specified." << std::endl;
         }
         else
         {
...
// #endif
      ...
      std::cout << *this << "** Before presolving: virtualMemUsedAtLc = " << virtualMemUsedAtLc << ", getVmSize() = " << getVmSize() << ", SCIPgetMemUsed() = " << SCIPgetMemUsed(scip) << ", SCIPgetMemTotal() = " << SCIPgetMemTotal(scip) << ", SCIPgetMemExternEstim() = " << SCIPgetMemExternEstim(scip) << std::endl;
#else
      virtualMemUsedAtLc = SCIPgetMemTotal(scip) + SCIPgetMemExternEstim(scip);
      std::cout << *this << "** Before presolving: virtualMemUsedAtLc = " << virtualMemUsedAtLc << ", SCIPgetMemUsed() = " << SCIPgetMemUsed(scip) << ", SCIPgetMemTotal() = " << SCIPgetMemTotal(scip) << ", SCIPgetMemExternEstim() = " << SCIPgetMemExternEstim(scip) << std::endl;
#endif
      ...
      {
         SCIP_CALL_ABORT( SCIPsetIntParam(scip, "presolving/maxrounds", 0) );
         std::cout << *this << "** No LC presolving is applied, since memory limit becomes "
         ...
         std::cout << *this << "=== solved at Init ===" << std::endl;
      ...
      std::cout << *this << "** set memory limit for presolving in LC to " << dynamic_cast<ScipParaParamSet *>(paraParams)->getRealParamValue(MemoryLimit) - virtualMemUsedAtLc*SCIP_MEMORY_COPY_FACTOR*paraComm->getSize() << " for SCIP **" << std::endl;
#else
      ...
      std::cout << *this << "** set memory limit for presolving in LC and solvers to " << dynamic_cast<ScipParaParamSet *>(paraParams)->getRealParamValue(MemoryLimit) << " for each SCIP **" << std::endl;
#endif
      ...
      std::cout << *this << "** Estimated virtualMemUsedAtSolver = " << virtualMemUsedAtLc << ", getVmSize() = " << vmSize << ", SCIPgetMemUsed() = " << SCIPgetMemUsed(scip) << ", SCIPgetMemTotal() = " << SCIPgetMemTotal(scip) << ", SCIPgetMemExternEstim() = " << SCIPgetMemExternEstim(scip) << std::endl;
#else
      virtualMemUsedAtLc = SCIPgetMemTotal(scip) + SCIPgetMemExternEstim(scip);
      std::cout << *this << "** Estimated virtualMemUsedAtSolver = " << virtualMemUsedAtLc << ", SCIPgetMemUsed() = " << SCIPgetMemUsed(scip) << ", SCIPgetMemTotal() = " << SCIPgetMemTotal(scip) << ", SCIPgetMemExternEstim() = " << SCIPgetMemExternEstim(scip) << std::endl;
#endif
      ...
      std::cout << *this << "** set memory limit for solvers to " << memoryLimitOfSolverSCIP << " for each SCIP **" << std::endl;
...
#endif
      std::cout << *this << "** Instance transfer method used: " <<  paraParams->getIntParamValue(InstanceTransferMethod) << std::endl;
      ...
      )
{
   ...
   if( isolname )
   {
      ...
      {
         std::cout << *this << "Warning: initial solution file name cannot rename: " << isolname << ", " << nRestartedRacing << "'th restarted file." << std::endl;
   ...
   if( !fp )
   {
      std::cout << *this << "Could not open " << generatedIsolname << " file to reinitialize for restart." << std::endl;
      abort();
   }
       ...
       scipStatus == SCIP_STATUS_INFEASIBLE )   // when sub-MIP is solved at root node, the solution may not be saved
   {
      ...
      setFinalSolverStatus(ProblemWasSolved);
      // finalDualBound = SCIPgetDualbound(scip);
      std::cout << *this << "=== solved at reInit ===" << std::endl;
      finalDualBound = instance->convertToInternalValue(SCIPgetDualbound(scip));
      writeSolution("Final Solution");
      return 1;
   }
      ...
      )
{
   ...
   */
      ...
      {
         std::cout << *this << "*** You should check the solution! ***" << std::endl;
         std::cout << *this << "checksol  = " << checksol << std::endl;
         std::cout << *this << "SCIPgetNVars(scip) = " << SCIPgetNVars(scip) << ", " << tempSol->getNVars() << std::endl;
         delete tempSol;
         return false;
      }
      assert(SCIPgetNVars(scip) <= tempSol->getNVars());
   ...
   if( paraParams->getBoolParamValue(CheckFeasibilityInLC) == false )
   {
      ...
      if( success && primalValueUpdated )
      {
         ...
         {
            std::cout << *this << "*** A new solution generated in a Solver does not match with the upperbound in LC ***" << std::endl;
            std::cout << *this << "SCIPgetUpperbound(scip) = " << SCIPgetUpperbound(scip) << std::endl;
            std::cout << *this << "solution->getObjectiveFunctionValue() = " << solution->getObjectiveFunctionValue() << std::endl;
            std::cout << *this << "Difference = " << (SCIPgetUpperbound(scip) - solution->getObjectiveFunctionValue())
   ...
   else
   {
      ...
      else
      {
         ...
         else
         {
            ...
            {
               // paraComm->lockApp();
               std::cout << *this << "Current scip primal value = " << SCIPgetPrimalbound(scip) << std::endl;
               std::cout << *this << "Objective value = " << convertToExternalValue(tempSol->getObjectiveFunctionValue()) << std::endl;
               std::cout << *this << "Initiator did not accept solution!" << std::endl;
               // paraComm->unlockApp();
            }
            SCIP_CALL_ABORT( SCIPfreeSol(scip, &newsol) );
            delete tempSol;
      ...
      )
{
   ...
   if( !file )
   {
      std::cout << *this << "file : " << filename << "cannot open." << std::endl;
      exit(1);
   }
   ScipParaInstance *scipInstance = dynamic_cast<ScipParaInstance *>(instance);
   if( scipInstance->isOriginalIndeciesMap() )
      ...
      )
{
   ...
   {
      std::cout << *this << "Checkpoint file for solution cannot open. file name = " << filename << std::endl;
      exit(1);
   }
   if( solution )
      ...
      )
{
   ...
   if( !checkpointSolutionStream )
   {
      std::cout << *this << "checkpoint solution file cannot open: file name = " <<  tempSolutionFileName << std::endl;
      exit(1);
   }
   if( solution )
   {
   ...
   if( solution )
   {
      ...
      {
         std::cout << *this << "***** Given solution is wrong! ***************************" << std::endl;
         std::cout << *this << "***** If the solution was given from checkpoint file,  ***" << std::endl;
         std::cout << *this << "***** it might be generated in original problem space   **" << std::endl;
         std::cout << *this << "***** Only primal value is used. *************************" << std::endl;
         std::cout << *this << "***** You should better to use -isol option.  ************" << std::endl;
         std::cout << *this << "***** Or, better to use no distribute solution option. ***" << std::endl;
      }
   }
   ...
   if( checkpointSolutionStream )
   {
      ...
      if( sol->read(paraComm, checkpointSolutionStream) )
      {
         if( !solution )
         {
            ...
            if( tryToSetIncumbentSolution(dynamic_cast<BbParaSolution *>(solution->clone(paraComm)), true) )
            {
               std::cout << *this << "***** After checkpoint solution is RIGHT! ****************" << std::endl;
            }
         }
         else
         {
      ...
      )
{
   ...
   if ( std::string(paraParams->getStringParamValue(UG::RacingParamsDirPath)) != std::string("") )
   {
      ...
      for( int n = 1; n < nParamSets + 1 ; n++ )
      {
         ...
         {
            std::cout << *this << "Cannot read racing parameter file = " << oss.str().c_str() << std::endl;
            exit(1);
         }
      ...
      )
{
   ...
   *os << *this << "Total Time         : " << time << std::endl;
   *os << *this << "  solving          : " << time << std::endl;
   *os << *this << "  presolving       : " << SCIPgetPresolvingTime(scip) << " (included in solving)" << std::endl;
   *os << *this << "B&B Tree           : " << std::endl;
   *os << *this << "  nodes (total)    : " << nSolved << std::endl;
   *os << *this << "Solution           : " << std::endl;
   *os << *this << "  Solutions found  : " << SCIPgetNSols(scip) << std::endl;
   ...
   *os << *this << "Gap                : ";
   ...
   {
      *os << std::scientific << *this << "* Warning: final gap: " << finalGap << " is greater than SCIPepsilon: " <<  SCIPepsilon(scip) << std::endl;
   }
...
{
   std::cout << *this << "Original Problem   : " << std::endl;
   std::cout << *this << "  Problem name     : " << SCIPgetProbName(scip) << std::endl;
   std::cout << *this << "  Variables        : " << SCIPgetNOrigVars(scip)
   ...
   std::cout << *this << "  Constraints      : " << SCIPgetNOrigConss(scip) << std::endl;
   std::cout << *this << "  Objective sense  : " <<  (SCIPgetObjsense(scip) == SCIP_OBJSENSE_MINIMIZE ? "minimize" : "maximize") << std::endl;
   std::cout << *this << "Presolved Problem  : " << std::endl;
   std::cout << *this << "  Variables        : " << SCIPgetNVars(scip)
   ...
   std::cout << *this << "  Constraints      : " << SCIPgetNConss(scip) << std::endl;

   std::cout << *this << "Constraints        : Number" << std::endl;
   for( int i = 0; i < SCIPgetNConshdlrs(scip); ++i )
   {
      ...
      {
         std::cout << *this << "  " << std::setw(17) << std::left << SCIPconshdlrGetName(conshdlr) << ": "
