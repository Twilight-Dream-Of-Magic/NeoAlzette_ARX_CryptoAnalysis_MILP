        ...
        pendingSolution(0)
{
   ...
   if(  paraParams->getBoolParamValue(UG::EnhancedFinalCheckpoint) )
   {
      ...
      {
         std::cout << *paraInitiator << "EnhancedCheckpointStartTime mast be greater than 1.0. " << std::endl;
         std::cout << *paraInitiator << "EnhancedCheckpointStartTime = " <<  paraParams->getRealParamValue(UG::EnhancedCheckpointStartTime) << std::endl;
         exit(1);
      }
      ...
      {
         std::cout << *paraInitiator << "EnhancedCheckpointStartTime mast be less than FinalCheckpointGeneratingTime. " << std::endl;
         std::cout << *paraInitiator << "EnhancedCheckpointStartTime = " <<  paraParams->getRealParamValue(UG::EnhancedCheckpointStartTime) << std::endl;
         std::cout << *paraInitiator << "FinalCheckpointGeneratingTime = " << paraParams->getRealParamValue(UG::FinalCheckpointGeneratingTime) << std::endl;
         exit(1);
      }
      if( paraParams->getIntParamValue(UG::FinalCheckpointNSolvers) > (paraComm->getSize() - 1) )
      {
         std::cout << *paraInitiator << "FinalCheckpointNSolvers mast be less equal than the number of solvers. " << std::endl;
         std::cout << *paraInitiator << "FinalCheckpointNSolvers = " << paraParams->getIntParamValue(UG::FinalCheckpointNSolvers) << std::endl;
         std::cout << *paraInitiator << "The number of solvers = " << (paraComm->getSize() - 1) << std::endl;
         exit(1);
      }
   }
   if ( paraParams->getRealParamValue(UG::FinalCheckpointGeneratingTime) > 0.0 )
   {
      ...
      std::cout << *paraInitiator << "** FinalCheckpointGeneratingTime is specified, then TimeLimit is omitted. ** " << std::endl;
   }

   // disply ramp-up mode
   if( paraParams->getIntParamValue(UG::RampUpPhaseProcess) == 0 )
   {
	   std::cout << *paraInitiator << "LC is working with normal ramp-up." << std::endl;
   }
   else if( paraParams->getIntParamValue(UG::RampUpPhaseProcess) == 1 )
   {
	   std::cout << *paraInitiator << "LC is working with racing ramp-up." << std::endl;
   }
   else if( paraParams->getIntParamValue(UG::RampUpPhaseProcess) == 2 )
   {
	   std::cout << *paraInitiator << "LC is working with racing ramp-up and with rebuilding tree after racing." << std::endl;
   }
   else if( paraParams->getIntParamValue(UG::RampUpPhaseProcess) == 3 )
   {
      std::cout << *paraInitiator << "LC is working with self-split ramp-up." << std::endl;
   }
   ...
   if( logSubtreeInfoFlag )
   {
      ...
      {
         std::cout << *paraInitiator << "Sub tree info. log file cannot open : file name = " << s.str() << std::endl;
         exit(1);
      }
   ...
   if( outputTabularSolvingStatusFlag || paraParams->getBoolParamValue(Quiet) )
   {
      ...
      else
      {
         ...
         {
            std::cout << *paraInitiator << "Tabular solving status file cannot open : file name = " << s.str() << std::endl;
            exit(1);
         }
...
//      }
   ...
   if( paraParams->getBoolParamValue(UG::GenerateReducedCheckpointFiles) )
   {
      ...
      {
         std::cout << *paraInitiator << "** -w opttion = " << paraInitiator->getPrefixWarm() << " **" << std::endl;
         std::cout << *paraInitiator << "** MergeNodesAtRestart = " << paraParams->getBoolParamValue(UG::MergeNodesAtRestart) << std::endl;
         std::cout << *paraInitiator << "** DualBoundGainTest = " << paraParams->getBoolParamValue(UG::DualBoundGainTest) << std::endl;
         std::cout << *paraInitiator << "** GenerateReducedCheckpointFiles is specified, then this solver shuld be executed with"
...
//   }
   ...
   {
      std::cout << *paraInitiator << lcts.toString() << std::endl;
   }

   if( osStatisticsFinalRun )
   {
      *osStatisticsFinalRun << *paraInitiator << lcts.toString();
   }

   if( paraRacingSolverPool && osStatisticsFinalRun )
   {
      *osStatisticsFinalRun << *paraInitiator << "***** <<< NOTE >>> "
            << paraRacingSolverPool->getNumActiveSolvers()
            << " active racing ramp-up solvers exist." << std::endl;
      *osStatisticsFinalRun << *paraInitiator << dynamic_cast<BbParaRacingSolverPool *>(paraRacingSolverPool)->getStrActiveSolerNumbers() << std::endl;
   }
      ...
      )
{
   ...
   if( updateSolution(sol) ) 
   {
      ...
      if( paraParams->getBoolParamValue(Checkpoint) && paraComm->getRank() == 0  )
      {
         ...
         {
            std::cout << *paraInitiator << "after checkpointing solution file cannot be renamed: errno = " << strerror(errno) << std::endl;
            exit(1);
         }
      }
#endif
      ...
      )
{
   ...
   if( tightenedIdex >= dynamic_cast<BbParaInstance *>(dynamic_cast<BbParaInitiator *>(paraInitiator)->getParaInstance())->getNVars() ) return 0;
   if( EPSLT(dynamic_cast<BbParaInitiator *>(paraInitiator)->getTightenedVarLbs(tightenedIdex), tightenedBound, MINEPSILON ) )
   {
      ...
      if( dynamic_cast<BbParaInitiator *>(paraInitiator)->setTightenedVarLbs(tightenedIdex, tightenedBound) &&
          ( paraRacingSolverPool && ( paraRacingSolverPool->getNumInactiveSolvers() == 0 ) ) )
      {
      ...
      )
{
   ...
   if( EPSGT(dynamic_cast<BbParaInitiator *>(paraInitiator)->getTightenedVarUbs(tightenedIdex), tightenedBound, MINEPSILON ) )
   {
      ...
      if( dynamic_cast<BbParaInitiator *>(paraInitiator)->setTightenedVarUbs(tightenedIdex, tightenedBound) &&
          ( paraRacingSolverPool && ( paraRacingSolverPool->getNumInactiveSolvers() == 0 ) ) )
      {
         for( size_t i = 1; i <= paraRacingSolverPool->getNumActiveSolvers(); i++ )
         {
...
{
   // output title line 1
   *osTabularSolvingStatus << *paraInitiator;
   ...
   for(;;)
   {
            ...
            )
      {
         if( runningPhase != TerminationPhase )
         {
            ...
            /* No active solver exists */
            terminateAllSolvers();
            std::cout << *paraInitiator << "### REACHED TO THE SPECIFIED NUMBER OF IDLER SOLVERS, then EXIT ###" << std::endl;
      ...
      )
{
   ...
   if( !paraNodePool->isEmpty() )
   {
      std::cout << *paraInitiator << "Logical error occurred during restart in ramp-down phase." << std::endl;
      std::cout << *paraInitiator << "You can restart from the chakepoint file." << std::endl;
      // exit(1);
      abort();
   }
               ...
               *osLogSolvingStatus << paraTimer->getElapsedTime()
               ...
               if( paraNode->getMergeNodeInfo() )
               {
                  *osLogSolvingStatus << " M(" << paraNode->getMergeNodeInfo()->nMergedNodes + 1 << ")";
                  ...
                  {
                     std::cout << *paraInitiator << "node id = " << (paraNode->getTaskId()).toString() << std::endl;
                     abort();
                  }
               }
               if( paraNode->getDiffSubproblem() )
      ...
      )
{
   ...
   {
      std::cout << *paraInitiator << "Checkpoint file for ParaNodes cannot open. file name = " << nodesFileName << std::endl;
      exit(1);
   }
   ...
   {
      std::cout << *paraInitiator << "Checkpoint file for SolverStatistics cannot open. file name = " << solverStatisticsFileName << std::endl;
      exit(1);
   }
   ...
   {
      std::cout << *paraInitiator << "Checkpoint file for SolverStatistics cannot open. file name = " << loadCoordinatorStatisticsFileName << std::endl;
      exit(1);
   }
   ...
   if( lastCheckpointTimeStr[0] == ' ' )
   {
      ...
      if( racingWinnerParams )
      {
         ...
         {
            std::cout << *paraInitiator << "Racing winner parameter file cannot open. file name = " << racingWinnerParamsName << std::endl;
            exit(1);
         }
   ...
   else
   {
      ...
      {
         std::cout << *paraInitiator << "checkpoint nodes file cannot be removed: errno = " << strerror(errno) << std::endl;
         exit(1);
      }
      if ( remove(solutionFileName) )
      {
         std::cout << *paraInitiator << "checkpoint solution file cannot be removed: errno = " << strerror(errno) << std::endl;
         exit(1);
      }
      if ( remove(solverStatisticsFileName) )
      {
         std::cout << *paraInitiator << "checkpoint SolverStatistics file cannot be removed: errno = " << strerror(errno) << std::endl;
         exit(1);
      }
      if ( remove(loadCoordinatorStatisticsFileName) )
      {
         std::cout << *paraInitiator << "checkpoint LoadCoordinatorStatistics file cannot be removed: errno = " << strerror(errno) << std::endl;
         exit(1);
      }
   }
   ...
   if( afterCheckpointingSolutionStream  )
   {
      ...
      {
         std::cout << *paraInitiator << "after checkpointing solution file cannot be removed: errno = " << strerror(errno) << std::endl;
         exit(1);
      }
   }
      ...
      )
{
   ...
   if( paraParams->getIntParamValue(RampUpPhaseProcess) == 1 || paraParams->getIntParamValue(RampUpPhaseProcess) == 2 )
   {
      if( !paraParams->getBoolParamValue(CollectOnce) )
      {
         std::cout << *paraInitiator << "*** When warm start with racing is specified, you should specify CollectOnce = TRUE ***" << std::endl;
         if( paraParams->getBoolParamValue(MergeNodesAtRestart) )
         {
            std::cout << *paraInitiator << "*** When warm start with racing is specified, you cannot specify MergeNodesAtRestart = TRUE ***" << std::endl;
         }
         exit(1);
      }
      if( paraParams->getBoolParamValue(MergeNodesAtRestart) )
      ...
      std::cout << *paraInitiator << "*** Warm start with racing ramp-up ***" << std::endl;
   }
   else
   {
      if( paraParams->getIntParamValue(RampUpPhaseProcess) == 3 )
      ...
      std::cout << *paraInitiator << "*** Warm start with normal ramp-up ***" << std::endl;
   }
   ...
   if( paraParams->getIntParamValue(RampUpPhaseProcess) == 0 )  // if it is not racing ramp-up
   {
      ...
      if( racingWinnerParamsStream )
      {
         ...
         std::cout << *paraInitiator << "*** winner parameter is read from " << racingWinnerParamsName << "***" << std::endl;
      }
   }
   ...
   for(;;)
   {
      ...
      {
         std::cout << *paraInitiator << "Checkpoint node has ancestor: " << paraNode->toString() << std::endl;
         std::cout << *paraInitiator << "Something wrong for this checkpoint file." << std::endl;
         exit(1);
      }
   ...
   if( paraParams->getIntParamValue(NNodesToKeepInCheckpointFile) > 0 )
   {
      ...
      {
         std::cout << *paraInitiator << "### NNodesToKeepInCheckpointFile is specified to "
      ...
      {
         std::cout << *paraInitiator << "### NNodesToKeepInCheckpointFile is specified to " << paraParams->getIntParamValue(NNodesToKeepInCheckpointFile) <<
      ...
      {
         std::cout << *paraInitiator << "### NEagerToSolveAtRestart is specified to "
               << paraParams->getIntParamValue(NNodesToKeepInCheckpointFile)
      ...
      std::cout << *paraInitiator << "### NNodesToKeepInCheckpointFile is specified to "
            << paraParams->getIntParamValue(NNodesToKeepInCheckpointFile)
            << " ###" << std::endl;
      std::cout << *paraInitiator << "### The number of no process nodes = " << paraNodeToKeepCheckpointFileNodes->getNumOfNodes()
            << " ###" << std::endl;
      std::cout << *paraInitiator << "### The number of nodes will be processed = " << paraNodePool->getNumOfNodes()
            << " ###" << std::endl;
   }
   ...
   if( paraParams->getIntParamValue(NEagerToSolveAtRestart) > 0 )
   {
      ...
      {
         std::cout << *paraInitiator << "### NEagerToSolveAtRestart is specified to " << paraParams->getIntParamValue(NEagerToSolveAtRestart) <<
               ", but the number of nodes in checkpoint file is " << tempParaNodePool.getNumOfNodes() << ". ###" << std::endl;
         exit(-1);
      }
      if( paraParams->getBoolParamValue(MergeNodesAtRestart) )
      ...
      std::cout << *paraInitiator << "### NEagerToSolveAtRestart is specified to "
            << paraParams->getIntParamValue(NEagerToSolveAtRestart)
            << " ###" << std::endl;
      std::cout << *paraInitiator << "### The number of no process nodes in the beginning = " << unprocessedParaNodes->getNumOfNodes()
            << " ###" << std::endl;
   }
      ...
      )
{
   ...
   if( !loadCoordinatorStatisticsStream )
   {
      std::cout << *paraInitiator << "checkpoint LoadCoordinatorStatistics file cannot open: file name = " <<  loadCoordinatorStatisticsFileName << std::endl;
      exit(1);
   }
   ...
   {
      std::cout << *paraInitiator << "checkpoint LoadCoordinatorStatistics file cannot read: file name = " <<  loadCoordinatorStatisticsFileName << std::endl;
      exit(1);
   }
   loadCoordinatorStatisticsStream.close();
   ...
   {
      std::cout << *paraInitiator << "checkpoint SolverStatistics file cannot open: file name = " <<  solverStatisticsFileName << std::endl;
      exit(1);
   }
   ...
   if( !ofsStatistics )
   {
      std::cout << *paraInitiator << "previous statistics file cannot open : file name = " << previousStatisticsFileName << std::endl;
      exit(1);
   }
   ...
   for( int i = 0; i < nSolverStatistics; i++ )
   {
      ...
      {
         std::cout << *paraInitiator << "checkpoint SolverStatistics file cannot read: file name = " <<  solverStatisticsFileName << std::endl;
         exit(1);
      }
      ...
      )
{
   ...
   // {
      ...
      if( paraParams->getBoolParamValue(StatisticsToStdout) )
      {
         std::cout << *paraInitiator << termState->toString(paraInitiator) << std::endl;
      }
...
// std::cout << "sendInterrruptRequest1 13" << std::endl;
         ...
         //      paraTimer->getElapsedTime() > paraParams->getRealParamValue(FinalCheckpointGeneratingTime) )
         {
            warmStartNodeTransferring = false;
            // hardTimeLimitIsReached = true;
            std::cout << *paraInitiator << "** Program is still in racing stage. FinalCheckpointGeneratingTime is sppecifid, but the checkpoint files would not be generated." << std::endl;
            // sendInterruptRequest();
            // runningPhase = TerminationPhase;     waits until paraRacingSolverPool becomes empty
            break;
         }
                 ...
                 EPSLT(bbParaInitiator->getGlobalBestIncumbentSolution()->getObjectiveFunctionValue(),DBL_MAX, DEFAULT_NUM_EPSILON ) ) ) )
         {
            ...
            else
            {
                    ...
                    )
               {
                  ...
                  winnerSolverNodesCollected = true;
                  std::cout << *paraInitiator << "Warning: Ramp-Up Phase Process is switched to 1. CollectOnce, MergeNodesAtRestart and RacingStatBranching are switched to FALSE." << std::endl;
                  std::cout << *paraInitiator << "You should check the following parameter values: StopRacingNumberOfNodesLeft, StopRacingNumberOfNodesLeftMultiplier, ProhibitCollectOnceMultiplier" << std::endl;
               }
