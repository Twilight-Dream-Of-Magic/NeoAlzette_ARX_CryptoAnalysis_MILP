...
call :BuildOne "1" "3" "Linear MILP search" "%REPO%\linear\neoalzette_scip_round_milp_search.cpp" "%REPO%\linear\neoalzette_scip_round_milp_search.exe"
if errorlevel 1 exit /b 1

call :BuildOne "2" "3" "Linear HULL search" "%REPO%\linear\neoalzette_scip_round_hull_search.cpp" "%REPO%\linear\neoalzette_scip_round_hull_search.exe"
if errorlevel 1 exit /b 1

call :BuildOne "3" "3" "Linear HULL multi-core runner" "%REPO%\linear\hull_multiple_thread_runner.cpp" "%REPO%\linear\hull_multiple_thread_runner.exe"
if errorlevel 1 exit /b 1

echo.
echo Linear build OK.
exit /b 0
