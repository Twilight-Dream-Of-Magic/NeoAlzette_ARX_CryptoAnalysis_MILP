# Linear HULL multi-process / multi-thread runner

`linear/hull_multiple_thread_runner.cpp` is the server campaign runner for the linear Forest/HULL search.
...
└─ a std::thread scheduler pool
   └─ one OS child process per active job
      └─ direct call to the linear Forest/HULL C++ API
...
- the two-variable modular-addition/subtraction linear MILP;
- the fixed-public-constant signed-transfer model;
- CLAT/Q1 correlation and sign verification;
- the joint-injection linear model;
...
The runner embeds the HULL implementation, so it must be linked with the same SCIP 10.0.2 include paths, libraries, and platform libraries used by the linear HULL executable.
...
./hull_multiple_thread_runner \
  ...
  --output runs/linear_seed_campaign \
  -- --rounds 1
```

Job `i` uses `--forest-seed (seed_start+i)`. Optional fixed input masks may be passed after `--`; when omitted, the linear HULL defaults remain:

```text
--fix-input-ma 0x00000001
--fix-input-mb 0x00000001
```

## Scheme B: fixed-input sector slicing

Scheme B fixes an externally selected input-mask pair `(mA,mB)` for each job. Within that job, the existing Forest Layer still feeds valid outputs back as later inputs until its time budget ends. After that child exits, another sector representative is scheduled.
...
./hull_multiple_thread_runner \
  ...
  --jobs 4096 \
  --job-time-limit 300 \
  --output runs/linear_sector_campaign \
  --prefix-bits 8 \
  --sector-start 0 \
...
With `--prefix-bits 8`, `(high8(mA),high8(mB))` defines `2^16` sectors. Sample zero uses zero low bits, except that the all-zero pair is repaired to a nonzero input. Additional samples use deterministic SplitMix64-derived low bits. These are representatives of a sector, not exhaustive enumeration of every pair in it.

## Runner-owned and forwarded options

Arguments after `--` are passed to the existing linear HULL parser inside the child API worker. The runner itself owns:

```text
--time-limit
--forest-seed
--hull-output-json
--hull-characteristics-jsonl
```
...
Repeating a runner-owned option after `--` is rejected rather than silently overridden. The shared C++ API entry performs the same `--time-limit` to Forest-total-budget transfer as the standalone linear HULL executable.
...
The summary reports observed unique `mA`, `mB`, input pairs, output pairs, prefix-cell coverage, Hamming-region coverage, maximum Forest layer, cycles, and best observed weights. Coverage means states actually present in worker Forest attempt records; it is not a claim of exhaustive coverage of the `2^64` pair space.

## HULL exactness boundary

Within each child, HULL correctness remains governed by the original MILP, Q1/CLAT verification, signed local correlations, semantic no-good constraints, and reoptimization.

The parent deliberately does **not** add signed correlations across jobs. Different workers can rediscover the same semantic characteristic, so a cross-worker numerical HULL sum would require a separate characteristic-level deduplication proof. Campaign-level aggregation is therefore limited to coverage and candidate statistics.
